...
export function PlansPage() {
  ...
  useEffect(() => { const saved = JSON.parse(localStorage.getItem(PLAN_KEY) ?? '[]') as PlanItem[]; setItems(saved); const meta = JSON.parse(localStorage.getItem(PLAN_META_KEY) ?? '{}') as { title?: string; date?: string }; setTitle(meta.title ?? 'Kế hoạch cuối tuần'); setDate(meta.date ?? ''); void Promise.all(saved.map(async (item) => { try { return await fetchPlaceDetail(API_URL, item.slug); } catch { return null; } })).then((rows) => setDetails(Object.fromEntries(rows.filter((row): row is PlaceDetail => row !== null).map((row) => [row.id, row])))); }, []);
