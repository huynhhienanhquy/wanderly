...
import { eventInputSchema, planningConstraintsSchema } from '@wanderly/contracts';
...
export class EventsController {
  ...
  @Post('recommendations') recommend(@Body() body: unknown) { const input = planningConstraintsSchema.safeParse(body); if (!input.success) throw new UnprocessableEntityException(input.error.flatten()); return this.events.recommend(input.data); }
